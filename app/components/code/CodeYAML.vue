...
onMounted(async () => {
  if (import.meta.client) {
    try {
      ...
      hljsCore.registerLanguage('yaml', yamlModule)
      hljsCore.registerLanguage('yml', yamlModule)
      
      const lines = props.code.split('\n')
      const highlighted = hljsCore.highlight(props.code, { language: 'yaml' }).value
      ...
      console.error('Error highlighting YAML code:', e)
      // Fallback
      const lines = props.code.split('\n')
      highlightedLines.value = lines.map((line, index) => ({
        code: line,
