...
onMounted(async () => {
  if (import.meta.client) {
    try {
      ...
      const properties = await import('highlight.js/lib/languages/properties')
      const hljsCore = hljs.default || hljs
      const propsModule = properties.default || properties
      hljsCore.registerLanguage('properties', propsModule)
      hljsCore.registerLanguage('env', propsModule)
      hljsCore.registerLanguage('.env', propsModule)
      
      const lines = props.code.split('\n')
      const highlighted = hljsCore.highlight(props.code, { language: 'env' }).value
      ...
      console.error('Error highlighting env code:', e)
      // Fallback
      const lines = props.code.split('\n')
      highlightedLines.value = lines.map((line, index) => ({
        code: line,
