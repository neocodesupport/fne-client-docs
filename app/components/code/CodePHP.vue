<template>
  ...
  >
    ...
    <pre
      ...
      :class="line.class"
    >
<code class="hljs" v-html="line.html"></code>
    </pre>
  </div>
</template>
...
onMounted(async () => {
  if (import.meta.client) {
    try {
      ...
      const phpModule = php.default || php
      hljsCore.registerLanguage('php', phpModule)
      
      const lines = props.code.split('\n')
      
      // Highlight chaque ligne individuellement pour préserver la structure HTML
      const highlightedLinesArray = await Promise.all(
        lines.map(async (line) => {
          if (!line.trim()) {
            return escapeHtml(line)
          }
          try {
            const result = hljsCore.highlight(line, { language: 'php' })
            return result.value
          } catch (e) {
            return escapeHtml(line)
          }
      ...
      // Debug: vérifier que le HTML est bien généré
      if (import.meta.dev) {
        console.log('Highlighted lines:', highlightedLinesArray.slice(0, 3))
        console.log('First line HTML:', highlightedLinesArray[0])
      }
      ...
      console.error('Error highlighting PHP code:', e)
      // Fallback
      const lines = props.code.split('\n')
      highlightedLines.value = lines.map((line, index) => ({
        code: line,
    ...
    // SSR - initialiser avec le code brut
    const lines = props.code.split('\n')
    highlightedLines.value = lines.map((line, index) => ({
      code: line,
      html: escapeHtml(line),
