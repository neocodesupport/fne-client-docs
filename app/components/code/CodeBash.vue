...
onMounted(async () => {
  if (import.meta.client) {
    try {
      ...
      const bashModule = bash.default || bash
      hljsCore.registerLanguage('bash', bashModule)
      hljsCore.registerLanguage('sh', bashModule)
      hljsCore.registerLanguage('shell', bashModule)
      
      const lines = props.code.split('\n')
      const highlighted = hljsCore.highlight(props.code, { language: 'bash' }).value
      ...
      console.error('Error highlighting bash code:', e)
      // Fallback
      const lines = props.code.split('\n')
      highlightedLines.value = lines.map((line, index) => ({
        code: line,
