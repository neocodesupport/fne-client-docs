...
onMounted(async () => {
  if (import.meta.client) {
    try {
      ...
      const xmlModule = xml.default || xml
      hljsCore.registerLanguage('xml', xmlModule)
      hljsCore.registerLanguage('html', xmlModule)
      
      const lines = props.code.split('\n')
      const highlighted = hljsCore.highlight(props.code, { language: 'xml' }).value
      ...
      console.error('Error highlighting HTML code:', e)
      // Fallback
      const lines = props.code.split('\n')
      highlightedLines.value = lines.map((line, index) => ({
        code: line,
